...
// R3F karakter sahnesi lazy load (varsa /public/hero/char.glb)
...
export default function Home() {
  ...
  // Scene mode (VIDEO default; R3F için asset gerekir)
  ...
  useEffect(() => {
    const v = (typeof window !== "undefined" && localStorage.getItem("sceneMode")) as
      | "VIDEO"
      | "R3F"
      | null;
    if (v) setSceneMode(v);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem("sceneMode", sceneMode);
  }, [sceneMode]);
  ...
  const hasFallback =
    !!result?.storyboard?.some(
      (f: any) =>
        String(f.caption).includes("(fallback)") || String(f.caption).includes("(placeholder)")
    );
      ...
      <ErrorBoundary>
        ...
        <div className="fixed top-4 right-4 z-50 glass flex items-center gap-2 px-3 py-2">
          ...
          <button
            ...
            className={`tag ${sceneMode === "VIDEO" ? "tag-active" : ""}`}
          >
            VIDEO
          </button>
          <button
            onClick={() => setSceneMode("R3F")}
            className={`tag ${sceneMode === "R3F" ? "tag-active" : ""}`}
          >
            R3F
          </button>
        </div>
        ...
        <div className="relative z-10 max-w-6xl mx-auto p-6 md:p-10 space-y-8">
          ...
          <div className="panel grid gap-4">
            ...
            {hasFallback && (
              <div className="text-xs bg-yellow-500/10 border border-yellow-500/30 text-yellow-200 px-3 py-2 rounded-lg">
                Üretimler şu an sınırlı modda (OpenAI erişimi bekleniyor). Görseller
                placeholder/fallback ile verildi.
              </div>
            )}
          </div>
          ...
          {result?.beats && (
            <div className="panel p-4">
              ...
              <BeatTimeline
                beats={result.beats}
                total={result.targetDurationSec}
                onPulse={trigger}
              />
            </div>
          )}
